...
int main() {
    ...
    maxlenftn maxf;
    for_each(names.begin(),names.end(),maxf).maxlen; // returns 0! does not work because it gets copied. fix by 
    cout << for_each(names.begin(),names.end(),maxf).maxlen << endl; // like so
    return 0;
}
