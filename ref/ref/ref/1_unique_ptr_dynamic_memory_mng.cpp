...
void exPreprocessor() {
    ...
    If memory is released too early, the program could crash from trying to use
    an object that no longer exists in the computer’s memory

    If memory is not released when it is no longer needed, the program can run
    out of memory (a memory leak)
    ...
    This is a special case of the most important idiom in C++, RAII
    ...
    managed the lifetimes of the object in
    memory
